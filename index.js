...
function getValues()
{
    ...
    if(document.getElementById('timequantum').value == "" && document.querySelector("#algo").value == "RR")
    {
...
function SRTF()
{
    ...
    while(ready.length > 0 || ind < n)
    {
        ...
        sortReadyBt();
        testReady();
        var idx = ready.shift(), bt = burstTime[idx];
        var flag = false;
...
function displayResult()
{
    ...
    avgTAT = avgTAT / turnAroundTime.length;

    if(waitingTime.length)
    avgWT = avgWT / waitingTime.length;
